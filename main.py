...
from bigquery_transfer import transfer, get_rows, count, delete
...
def delete_records_if_exist(where_clause: str,*, timeout: int = 60, dry_run: bool = False):
    deleted = 0
    record_count = count(where_clause, timeout=timeout)
    if record_count > 0:
        deleted = delete(where_clause, timeout=timeout, dry_run=dry_run)

    if deleted is None:
        deleted = record_count - (count(where_clause, timeout=timeout))

    return record_count, deleted
...
if __name__ == "__main__":
    ...
    notify_on_exception(delete_records_if_exist, silent=True)(f"day = {DATE_TO_FETCH.strftime('%Y-%m-%d')}")
